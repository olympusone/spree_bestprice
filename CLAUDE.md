...
- **`app/models/spree/integrations/bestprice.rb`** — integration model (inherits `Spree::Integration`), grouped under `'marketing'`
- **`app/controllers/spree/bestprice_controller.rb`** — inherits `Spree::StoreController`; checks `store_integration('bestprice')` before serving the feed; responds to `.xml` and `.xml.gz` formats
...
- **`app/views/spree/admin/integrations/forms/_bestprice.html.erb`** — admin UI partial shown when configuring the integration
...
| `title`                      | `product.name`, or `"#{product.name} - #{color}"` for a color entry                                                                                                                                                                                                                                              |
